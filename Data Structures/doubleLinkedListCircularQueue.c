...
void display(){
	...
	if(head == NULL){
		...
		if(head -> nextAddress != head)
			printf("\n last but one element is %d", temporaryNode -> previousAddress -> number);
	}
}
...
void insert(){
	...
	scanf("%d", &value);
	if(head == NULL){
		temporaryNode -> previousAddress = head;
...
void delete(){
	...
	if(head == NULL){
		...
		head -> previousAddress = temporary;
		temporary -> nextAddress = head;
		free(temporaryNode);
	}
}
